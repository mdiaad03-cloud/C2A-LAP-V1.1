...
import { buildAgentSupportReply } from "../services/operationsAgentService.js";
import {
  asOptionalText,
  requireEmail,
  requireText,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (agentSettings.supportReplyEnabled !== false) {
      try {
        let linkedOrder = null;
        if (ticket.orderNumber) {
          linkedOrder = db.onlineOrders.find(
            (o) => String(o.orderNumber).toLowerCase() === String(ticket.orderNumber).toLowerCase()
          );
        }
        if (!linkedOrder && ticket.customerEmail) {
          linkedOrder = db.onlineOrders
            .filter((o) => String(o.customerEmail).toLowerCase() === String(ticket.customerEmail).toLowerCase())
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
        }

        const agentReply = buildAgentSupportReply({ ticket, linkedOrder, settings: agentSettings, db });
        const bodyText = ticket.messages[0]?.body || "";
        const hasArabic = /[\u0600-\u06FF]/.test(bodyText);
        const replyText = hasArabic ? agentReply.replyAr : agentReply.reply;
        ...
        ticket.status = agentReply.recommendedStatus || "in_progress";
        ticket.updatedAt = nowIso();
        await saveDb();
        console.log(`[AI Support Agent] Auto-replied to ticket ${ticket.id}`);
      } catch (agentErr) {
        console.error("AI Support Agent auto-reply failed during ticket creation:", agentErr.message);
      }
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Auto AI response if customer replied and support agent is enabled
    if (req.user.role === "customer") {
      const agentSettings = db.storeSettings?.agentSettings || {};
      if (agentSettings.supportReplyEnabled !== false) {
        try {
          ...
          ticket.updatedAt = nowIso();
          await saveDb();
          console.log(`[AI Support Agent] Auto-replied to customer message on ticket ${ticket.id}`);
        } catch (agentErr) {
          console.error("AI Support Agent auto-reply failed during customer reply:", agentErr.message);
        }
      }
    }
