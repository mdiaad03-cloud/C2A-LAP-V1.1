...
import { requireMinLength, requireText } from "../utils/validation.js";
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      username = requireText(req.body.username, "Username");
      password = requireMinLength(req.body.password, 1, "Password");
    } catch (error) {
      return res.status(400).json({
        message: error.message || "Username and password are required.",
        error: error.message || "Username and password are required.",
      });
    }
