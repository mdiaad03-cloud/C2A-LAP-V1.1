...
const paymobApi = axios.create({ baseURL: "/api/paymob", timeout: 20000 });
...
function getErrorMessage(error, defaultMsg) {
  ...
  if (data && typeof data === "object") {
    ...
    if (errVal) {
      ...
      if (typeof errVal.detail === "string") return errVal.detail;
...
function createPlaceholder(productOrText, variant = 0) {
  const palettes = [
    ["#11253b", "#f97316"],
    ["#032d3c", "#10b981"],
    ["#1f2937", "#3b82f6"],
  ...
  if (productOrText && typeof productOrText === "object") {
    brand = String(productOrText.brand || "C2A LAP");
    model = String(productOrText.laptopName || "Laptop");
  } else if (typeof productOrText === "string") {
    const parts = productOrText.trim().split(/\s+/);
    brand = parts[0] || "C2A LAP";
    model = parts.slice(1).join(" ") || "Laptop";
  }
  
  brand = brand.replace(/&/g, "and");
...
function StoreAdaptiveImage({ src, alt, className = "", profile = "default" }) {
  ...
  function handleError() {
    const fallback = createPlaceholder(alt, 0);
    if (imgSrc !== fallback) {
      setImgSrc(fallback);
    }
  }
  ...
  const scaleByProfile = {
    detail: { portrait: 1, square: 1, landscape: 1 },
    slide: { portrait: 1, square: 1, landscape: 1 },
    card: { portrait: 1, square: 1, landscape: 1 },
    compact: { portrait: 1, square: 1, landscape: 1 },
    default: { portrait: 1, square: 1, landscape: 1 },
  };
...
function StoreProvider({ children }) {
  ...
  async function refreshStore() {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, "Store is unavailable."));
    } finally {
      setLoading(false);
    }
  }
...
async function placeOrder({ customer, paymentMethod, paymentReference }) {
  ...
  // PAYMOB PAYMENT
  // =========================
  if (paymentMethod === "paymob_egypt") {
    const paymobResponse = await paymobApi.post(
      "/create-payment",
      {
        currency,
        paymentMethod,
    ...
    console.log("PAYMOB RESPONSE:", paymobResponse.data);

    if (paymobResponse.data?.iframeUrl) {
      console.log("PAYMOB REDIRECT");
      // Direct redirect to the Paymob iframe payment page
      window.location.href = paymobResponse.data.iframeUrl;

      return {
        success: true,
        paymentUrl: paymobResponse.data.iframeUrl,
      };
    }
...
function StoreProductDetailsPage() {
  ...
  async function submitReview(event) {
    ...
    try {
      ...
      toast.error(
        getErrorMessage(error, tr("Failed to submit review.", "\u062a\u0639\u0630\u0631 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u062a\u0642\u064a\u064a\u0645.")),
      );
    } finally {
    ...
    <div className="store-cart-layout">
      ...
      <aside className="store-section store-summary">
        ...
        <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem" }}>
          <input
            type="text"
            placeholder={tr("Promo Code", "كود الخصم")}
            value={promoInput}
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                const response = await storeApi.get(`/discount/check/${code}`, {
                  headers: customerHeaders(),
                });
...
function StorePaymentPage() {
  ...
  async function confirmPayment(event) {
    ...
    try {
      ...
      const errorMsg = getErrorMessage(error, tr("Payment step failed. Try again.", "\u0641\u0634\u0644\u062a \u062e\u0637\u0648\u0629 \u0627\u0644\u062f\u0641\u0639. \u062d\u0627\u0648\u0644 \u0645\u0631\u0629 \u0623\u062e\u0631\u0649."));
...
function StoreAccountPage() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Account request failed."));
    } finally {
      setAuthSaving(false);
    }
  }
  ...
  async function handleAvatarUpload(file, target) {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, tr("Could not upload image.", "\u062a\u0639\u0630\u0631 \u0631\u0641\u0639 \u0627\u0644\u0635\u0648\u0631\u0629.")));
    } finally {
      setUploading(false);
    }
  }
  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      try {
        ...
        toast.error(getErrorMessage(error, tr("Could not complete social login.", "\u062a\u0639\u0630\u0631 \u0625\u0643\u0645\u0627\u0644 \u062a\u0633\u062c\u064a\u0644 \u0627\u0644\u062f\u062e\u0648\u0644 \u0627\u0644\u0627\u062c\u062a\u0645\u0627\u0639\u064a.")));
      } finally {
...
function StoreSupportPage() {
  ...
  async function submitTicket(event) {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, tr("Could not send support ticket.", "\u062a\u0639\u0630\u0631 \u0625\u0631\u0633\u0627\u0644 \u0631\u0633\u0627\u0644\u0629 \u0627\u0644\u062f\u0639\u0645.")));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function submitReply(ticketId) {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, tr("Reply failed.", "\u0641\u0634\u0644 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u0631\u062f.")));
    } finally {
      setReplyingTicketId("");
    }
  }
...
function StoreSuccessPage() {
  ...
  const { formatPrice, tr, customerOrders, refreshCustomerOrders, clearCart } = useStore();
  ...
  useEffect(() => {
    ...
    // Fetch single order details to prevent empty fields on page refresh / external redirect
    async function loadOrder() {
      setFetching(true);
      try {
        const response = await customerApi.get(`/orders/${orderNumber}`, {
          headers: customerHeaders(),
        });
  ...
  const order = location.state?.order || fetchedOrder || customerOrders.find((o) => o.orderNumber === orderNumber);
  const customerName = location.state?.customerName || order?.customerName || tr("Customer", "العميل");
  ...
  const paymentStatusLabel = (status) => {
    const map = {
      pending_collection: tr("Pending Collection", "بانتظار التحصيل"),
      paid: tr("Paid", "مدفوع"),
      pending_verification: tr("Pending Verification", "بانتظار التحقق"),
      failed: tr("Failed", "فشل"),
    };
    return map[status] || status || tr("Pending", "معلق");
  };

  return (
    <section className="store-section success-state">
      <BadgeCheck size={48} style={{ color: "var(--accent)" }} />
      <h2 style={{ marginTop: "12px" }}>{tr("Order Placed Successfully!", "تم تقديم الطلب بنجاح!")}</h2>
      <p style={{ color: "var(--muted)", marginBottom: "24px" }}>
        {tr("Thank you", "شكرا لك")} <strong>{customerName}</strong>. {tr("Your order number is", "رقم طلبك هو")} <strong>{orderNumber}</strong>.
      </p>

      {fetching && !order && (
        <p style={{ color: "var(--muted)" }}>{tr("Loading order details...", "جارٍ تحميل تفاصيل الطلب...")}</p>
      )}

      {order && (
        <div style={{ width: "100%", maxWidth: "560px", margin: "0 auto", display: "grid", gap: "16px" }}>
          {/* Customer & Delivery */}
          <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "12px", padding: "16px", display: "grid", gap: "8px" }}>
            <h4 style={{ margin: 0, fontSize: "0.95rem", color: "var(--muted)", fontWeight: 600 }}>{tr("Delivery Info", "معلومات التوصيل")}</h4>
            <p style={{ margin: 0 }}><strong>{tr("Name", "الاسم")}:</strong> {order.customerName}</p>
            <p style={{ margin: 0 }}><strong>{tr("Phone", "الهاتف")}:</strong> {order.customerPhone}</p>
            {order.customerAddress && (
              <p style={{ margin: 0 }}><strong>{tr("Address", "العنوان")}:</strong> {order.customerAddress}{order.customerCity ? `, ${order.customerCity}` : ""}</p>
            )}
            {order.customerNotes && (
              <p style={{ margin: 0 }}><strong>{tr("Notes", "ملاحظات")}:</strong> {order.customerNotes}</p>
            )}
          </div>
          ...
          {Array.isArray(order.items) && order.items.length > 0 && (
            <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "12px", padding: "16px", display: "grid", gap: "8px" }}>
              <h4 style={{ margin: 0, fontSize: "0.95rem", color: "var(--muted)", fontWeight: 600 }}>{tr("Order Items", "عناصر الطلب")}</h4>
              {order.items.map((item, idx) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px" }}>
                  <span style={{ flex: 1, fontSize: "0.9rem" }}>
                    {item.laptopName || item.brand || tr("Item", "منتج")}
                    {item.ram ? ` – ${item.ram}` : ""}
          ...
          {/* Price Summary */}
          <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "12px", padding: "16px", display: "grid", gap: "8px" }}>
            <h4 style={{ margin: 0, fontSize: "0.95rem", color: "var(--muted)", fontWeight: 600 }}>{tr("Price Summary", "ملخص السعر")}</h4>
            ...
            {Number(order.discountAmount || 0) > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", color: "#10b981" }}>
                <span>{tr("Discount", "الخصم")} {order.discountCode ? `(${order.discountCode})` : ""}</span>
                <span>-{formatPrice(order.discountAmount)}</span>
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: "1.1rem", borderTop: "1px solid var(--line)", paddingTop: "8px", marginTop: "4px" }}>
              <span>{tr("Total", "الإجمالي")}</span>
          ...
          {/* Payment Status */}
          <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "12px", padding: "16px", display: "grid", gap: "8px" }}>
            <h4 style={{ margin: 0, fontSize: "0.95rem", color: "var(--muted)", fontWeight: 600 }}>{tr("Payment", "الدفع")}</h4>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>{tr("Method", "الطريقة")}</span>
              <span>{paymentMethodLabel(order.paymentMethod)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>{tr("Status", "الحالة")}</span>
              <span style={{ color: order.paymentStatus === "paid" ? "#10b981" : "inherit" }}>
                {paymentStatusLabel(order.paymentStatus)}
              </span>
            </div>
            {order.createdAt && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span>{tr("Placed at", "وقت الطلب")}</span>
      ...
      <div className="store-hero-actions" style={{ marginTop: "24px" }}>
        <Link to="/store/products" className="store-primary-btn">
